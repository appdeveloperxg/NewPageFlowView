...
//    if (point.y>=_scrollView.frame.origin.y&&point.y<=_scrollView.frame.size.height) {
   
    if (point.x <_scrollView.frame.origin.x ) {
        
        CGFloat  num = (_scrollView.frame.origin.x - point.x)/_pageSize.width;
        NSInteger page = 0;
        page = (NSInteger)num +1;
        if (self.page>page-1) {
            
            [self scrollToPage:self.page-page];
        }
        
    }else if(point.x >_scrollView.frame.origin.x+_scrollView.frame.size.width)
    {
        CGFloat  num = (point.x -_scrollView.frame.origin.x - _scrollView.frame.size.width)/_pageSize.width;
        NSInteger page=0;
        page = (NSInteger)num +1;
        if ((self.orginPageCount-self.page-1)>page-1) {
...
//    NSLog(@"startPoint.x===========%f,endPoint.x============%f",startPoint.x,endPoint.x);
...
//            NSLog(@"self.visibleRange.location===========%ld,self.visibleRange.length===========%ld",self.visibleRange.location,self.visibleRange.length);
